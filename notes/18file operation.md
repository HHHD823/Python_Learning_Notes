...
## 写文件操作
把程序输出结果写成文件,就可以不用反复运行程序,也可以把结果轻松共享给他人
1. 打开文件 → 和读文件操作类似
- 第二个参数用w 写入模式
![img.png](../assets/img18_12.png)
- 和读取模式不一样的是,如果写入的文件不存在,程序会自动创建传入文件名的那个文件
- 第三个参数encoding
![img_1.png](../assets/img18_13.png)
- **用w模式打开文件进行写入的话,如果那个文件已经存在,就会把原本的文件内容清空,所以用w模式前要三思而后行**

2. 写文件
### w写入模式
- write()
![img_2.png](../assets/img18_14.png)
不会每次调用时默认换行,需要手动添加换行符.
### a附加模式
- 如果不想写文件时把原文件内容清空,而是在后面继续写,那么模式参数应该用"a"
![img_3.png](../assets/img18_15.png)
- 若文件名不存在会直接创建一个


![img_4.png](../assets/img18_16.png)
在w和a模式下,不支持read这一读取文件功能,怎么办?

### "r+"模式
可以同时支持读写.而且write调用后,会以追加形式在文件后面添加新内容
![img_5.png](../assets/img18_17.png)

[practice](/code/18_2.py)